                ...
                while read domain; do
                        ...
                        for i in "${proto[@]}"; do
                                ...
                                if [[ $(curl --connect-timeout 2.37 -H 'Connection: close' $i${url} -I -s | head -n 1 | cut '-d ' '-f2') == 200 ]]; then
                                        echo "$i$url";
                                elif [[ $(curl --connect-timeout 2.37 -H 'Connection: close' $i${url} -I -s | head -n 1 | cut '-d ' '-f2') == 301 ]]; then
                                        location=$(curl -s --connect-timeout 2.37 -H 'Connection: close' $i${url} -IL | grep 'Location: '| cut '-d ' '-f2'| tail -1)
                                        echo "Redirect found for $i$url : $location";
                                fi
